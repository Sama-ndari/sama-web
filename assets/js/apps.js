...
function appScreenshots(id, count) {
  var shots = [];
  for (var i = 1; i <= count; i++) {
    shots.push(appAsset(id, "screenshots/preview-" + i + ".webp"));
  }
  return shots;
}
...
var APPS = [
  {
    ...
    icon: appAsset("yimburundi", "icon.webp"),
    screenshots: appScreenshots("yimburundi", 7),
    videos: [],
    shortDesc:
  ...
  },
  {
    ...
    icon: appAsset("ibanga", "icon.webp"),
    screenshots: appScreenshots("ibanga", 6),
    videos: [],
    shortDesc:
...
var SAMA_WEB_CATALOG = {
  websiteCount: APPS.filter(function (app) {
    return app.productType === "website";
  }).length
};
